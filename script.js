...
window.addEventListener('scroll', () => {
  ...
  pb.style.width = (winScroll / height) * 100 + '%';
});
...
(function () {
  ...
  function render(t) {
    ...
    gl.uniform2f(uM, bx, cv.height - by);
...
(function () {
  ...
  class P {
    reset() {
      ...
      const r = 110 + Math.random() * 100;
      this.x = innerWidth / 2 + Math.cos(a) * r;
      this.y = innerHeight / 2 + Math.sin(a) * r;
      this.vx = (Math.random() - 0.5) * 0.6;
      this.vy = (Math.random() - 0.5) * 0.6;
      this.life = 1;
      this.size = Math.random() * 3 + 1;
      this.col = C[Math.floor(Math.random() * 4)];
    }
    constructor() {
      this.reset();
    }
    update() {
      this.x += this.vx + (imx - innerWidth / 2) * 0.0003;
      this.y += this.vy + (imy - innerHeight / 2) * 0.0003;
      this.life -= 0.007;
  ...
  const ps = [];
  for (let i = 0; i < 140; i++) ps.push(new P());
...
function triggerExplode() {
  ...
  prof.style.cssText =
    'transform:scale(0);opacity:0;transition:transform .5s cubic-bezier(.55,0,1,.45),opacity .3s';
  const f = Object.assign(document.createElement('div'), {
    style: 'position:absolute;inset:0;background:#fff;z-index:20',
  });
  intro.appendChild(f);
  setTimeout(() => {
    f.style.cssText =
      'opacity:0;transition:opacity .6s;position:absolute;inset:0;background:#fff;z-index:20';
  }, 100);
  setTimeout(() => {
    intro.style.cssText =
      'transition:opacity .8s,transform .8s cubic-bezier(.16,1,.3,1);opacity:0;transform:scale(1.1);position:fixed;inset:0;background:#000;z-index:1000;display:flex;align-items:center;justify-content:center;flex-direction:column';
...
setTimeout(() => {
  if (document.getElementById('intro-screen').style.display !== 'none') triggerExplode();
}, 5000);

// ── SKILL CAROUSEL DATA (SOLO LAS 7 TECNOLOGÍAS) ──
const row1 = [
  {
    name: 'Docker',
    color: '#2496ED',
  ...
  },
  {
    name: 'JavaScript',
    color: '#F7DF1E',
    svg: `<svg viewBox="0 0 24 24"><rect width="24" height="24" fill="#F7DF1E" rx="2"/><path d="M6.5 5.8l1.6-.003 1.23 6.9 1.43-6.9h1.6l-2.19 9.38H8.35zm7.54 0h1.57v6.26c0 1.05-.05 1.72-.14 2.01-.18.56-.55.85-1.11.85-.28 0-.55-.05-.79-.16l.25-1.26c.14.07.26.1.35.1.17 0 .29-.06.36-.17.07-.1.1-.47.1-1.07zm2.67 7.09 1.53-.24c.07.32.2.56.4.72.2.16.48.24.85.24.4 0 .7-.08.91-.25.14-.11.2-.26.2-.44a.45.45 0 00-.12-.32c-.08-.09-.29-.17-.62-.25-1.49-.33-2.44-.63-2.84-.9-.57-.38-.85-.9-.85-1.57 0-.6.23-1.1.69-1.52.46-.42 1.15-.63 2.06-.63.88 0 1.56.17 2.02.5.46.34.77.84.94 1.51l-1.44.27c-.06-.24-.18-.42-.36-.55-.18-.13-.43-.2-.74-.2-.4 0-.69.06-.86.19a.35.35 0 00-.13.28c0 .1.05.18.14.25.12.09.58.23 1.37.41.79.18 1.35.4 1.67.66.32.26.48.63.48 1.1 0 .63-.25 1.16-.75 1.61-.5.45-1.23.67-2.19.67-.88 0-1.59-.19-2.11-.56-.52-.37-.86-.88-1.01-1.53z" fill="#000"/></svg>`,
  },
  {
    name: 'Java',
    color: '#007396',
...
const row2 = [...row1]; // repetimos las mismas para el segundo carrusel (puedes duplicar o variar)

function buildCarousel(el, skills) {
  [...skills, ...skills].forEach((s) => {
    const chip = document.createElement('div');
    chip.className = 'skill-chip';
    chip.addEventListener('mouseenter', () => {
      chip.style.borderColor = s.color + '99';
      chip.style.boxShadow = `0 12px 40px rgba(0,0,0,.5),0 0 22px ${s.color}55`;
    });
    chip.addEventListener('mouseleave', () => {
      chip.style.borderColor = '';
      chip.style.boxShadow = '';
    });
    chip.innerHTML = `<div class="skill-chip-logo">${s.svg}</div><div class="skill-chip-name">${s.name}</div>`;
    el.appendChild(chip);
  });
}

buildCarousel(document.getElementById('carousel-1'), row1);
buildCarousel(document.getElementById('carousel-2'), row2);
...
document.querySelectorAll('.tilt-card').forEach((card) => {
  card.addEventListener('mousemove', (e) => {
    ...
    card.style.transform = `perspective(600px) rotateY(${x * 14}deg) rotateX(${
      -y * 14
    }deg) scale(1.02)`;
